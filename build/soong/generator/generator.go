...
func init() {
	android.RegisterModuleType("phantom_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = phantomExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = phantomExpandVariables(ctx, dep_file)
	...
	cmd := phantomExpandVariables(ctx, String(g.properties.Cmd))
