...
function setup_vendor() {
    ...
    export PHANTOM_ROOT="$3"
    if [ ! -d "$PHANTOM_ROOT" ]; then
        echo "\$PHANTOM_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$PHANTOM_ROOT/$OUTDIR" ]; then
        mkdir -p "$PHANTOM_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$PHANTOM_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$PHANTOM_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$PHANTOM_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            ...
            printf "# Copyright (C) $YEAR The Phantom Open Sauce Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR The Phantom Open Sauce Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The Phantom Open Sauce Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR The Phantom Open Sauce Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$PHANTOM_ROOT"/vendor/phantom/build/tools/smali/baksmali.jar
        export SMALIJAR="$PHANTOM_ROOT"/vendor/phantom/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$PHANTOM_ROOT"/vendor/phantom/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$PHANTOM_ROOT"/vendor/phantom/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$PHANTOM_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$PHANTOM_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$PHANTOM_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$PHANTOM_ROOT"/vendor/phantom/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$PHANTOM_ROOT"/"$OUTDIR"/radio
