function __print_phantom_functions_help() {
cat <<EOF
Additional PhantomROM functions:
...
function breakfast()
{
    target=$1
    local variant=$2
    PHANTOM_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    for f in `/bin/ls vendor/phantom/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Phantom model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch phantom_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/phantom-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.phantom.device | grep -q "$PHANTOM_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $PHANTOM_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function gerritpush()
{
    ...
    GERRIT_URL=review.phantomproject.co;
    ...
    then
      local PFX="PhantomProject/";
    else
      local PFX="PhantomDevices/";
    fi
    cd $c;
...
function installboot()
{
    ...
    if (adb shell getprop ro.phantom.device | grep -q "$PHANTOM_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        phantomremote
        git push phantom HEAD:refs/heads/'$1'
    fi
    '
}
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                phantom|otapackage|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
...
function deleteOTA() {
    ...
    curl --header "Content-Type: application/json" \
        ...
        --request POST \
        --data "$data" \
        https://api.phantomproject.co/deleteUpdate 2> /dev/null | \
...
function pushOTA() {
    ...
    file=$(ls -t ${OUT}/phantom_${PHANTOM_BUILD}-9* | sed -n 2p);
    URL="https://sourceforge.net/projects/posp/files/$PHANTOM_BUILD/weeklies/${file##*/}";
    ...
    size=$(stat -c%s $file);
    version=$(grep ro\.phantom\.version $OUT/system/build.prop | cut -d= -f2);
    data="{\"datetime\":\"$datetime\", \"devicename\":\"$PHANTOM_BUILD\",\"filename\":\"${file##*/}\",\"id\":\"$id\",\"romtype\":\"$romtype\",\"size\":\"$size\",\"url\":\"$url\",\"version\":\"$version\"}";

    curl --header "Content-Type: application/json" \
        --user $OTA_API_USER:$OTA_API_SECRET \
        --request POST \
        --data "$data" \
        https://api.phantomproject.co/pushUpdate 2> /dev/null | \
        python -c "import sys, json; print json.load(sys.stdin)['response']";
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.phantom.device | grep -q "$PHANTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/phantom/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $PHANTOM_FIXUP_COMMON_OUT ]; then
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/phantom/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
